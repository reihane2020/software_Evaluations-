...
class CommentEvaluateViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if publish:
            ...
            if res.count() >= 1 and not "comment" in self.request.user.can_publish_evaluation:
                raise APIException(
                    code="LIMIT_EVALUATION",
                    detail="You can publish 1 evaluation per month"
                )

            if "comment" in self.request.user.can_publish_evaluation:
                self.request.user.can_publish_evaluation.remove("comment")
